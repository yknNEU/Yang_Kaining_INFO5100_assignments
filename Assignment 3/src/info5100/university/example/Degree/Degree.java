...
public class Degree {
    ...
    public boolean isStudentReadyToGraduate(StudentProfile sp) {
        ...
        if (getTotalElectiveCoursesTaken(sas) < electives.size()) {
            return false;
        } // electives satisfied
      
        // Check for the total number of credit hours that it is above 32
        int totalCreditHours = 0;
        // TODO: Check if the student has at least 32 credit hours
        for (Course c : corelist) {
            totalCreditHours = totalCreditHours + c.getCredits();
        }
        for (Course c : electives) {
            totalCreditHours = totalCreditHours + c.getCredits();
        }
        if (totalCreditHours < 32) {
            return false;
        } // total credit hours satisfied

        else {
            return true;
        } // student has at least 32 credit hours per NEU requirements
    }
